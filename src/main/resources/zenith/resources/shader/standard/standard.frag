...
vec2 displaceTexCoord(vec2 texCoord, vec3 viewDirectionT, sampler2D heightMap, float heightScale) {
  float numHeightLayers = mix(maxHeightLayers, minHeightLayers, abs(dot(vec3(0.0, 0.0, 1.0), viewDirectionT)));
  ...
  float heightBeforeCollision = texture(heightMap, prevTexCoord).r - currentLayerHeight + layerHeight;
  float heightAfterCollision = currentHeightMapValue - currentLayerHeight;

  float t = heightAfterCollision / (heightAfterCollision - heightBeforeCollision);
...
float calcAttenuation(float radius, float distance) {
	...
	return pow(clamp(1.0 - pow(distance / radius, 4.0), 0.0, 1.0), 2.0) / (distance * distance + 1);
}
...
vec3 calcLuminance(vec3 positionT, vec3 normalT, vec3 viewDirectionT, vec3 lightDirectionT, vec3 radiance, vec3 albedo, float metallic, float roughness) {
	...
	vec3 f = fresnelSchlick(mix(vec3(F0), albedo, metallic), clamp(vh, 0.0, 1.0));
...
vec3 calcDirectionalLight(DirectionalLight directionalLightT, vec3 positionT, vec3 normalT, vec3 viewDirectionT, vec3 albedo, float metallic, float roughness) {
	vec3 lightDirectionT = -directionalLightT.direction;

	vec3 radiance = directionalLightT.light.color;
	
	return calcLuminance(positionT, normalT, viewDirectionT, lightDirectionT, radiance, albedo, metallic, roughness);
}

vec3 calcPointLight(PointLight pointLightT, vec3 positionT, vec3 normalT, vec3 viewDirectionT, vec3 albedo, float metallic, float roughness) {
...
vec3 calcSpotLight(SpotLight spotLightT, vec3 positionT, vec3 normalT, vec3 viewDirectionT, vec3 albedo, float metallic, float roughness) {
	vec3 lightDirectionT = normalize(spotLightT.pointLight.position - positionT);
	...
	float epsilon = abs(cos(spotLightT.innerCutoffAngle) - cos(spotLightT.outerCutoffAngle));
	
	if (epsilon == 0.0) {
		epsilon = 0.001;
	}
	
	float intensity = clamp((theta - cos(spotLightT.outerCutoffAngle)) / epsilon, 0.0, 1.0);
...
void main()
{
  ...
  if (material.useHeightMap) {
    texCoord = displaceTexCoord(fragTexCoord, fragViewDirectionT, material.heightMap, material.heightScale);
  } else {
  ...
  if (material.useNormalMap) {
    normalT = normalize(texture(material.normalMap, texCoord).rgb * 2.0 - 1.0);
  } else {
    normalT = fragNormalT;
  }
  ...
  int maxFragDirectionalLights = min(numFragDirectionalLights, MAX_NUM_FRAG_LIGHTS);
  for (int i = 0; i < maxFragDirectionalLights; i++) {
  	if (numLights < MAX_NUM_FRAG_LIGHTS) {
  		result += calcDirectionalLight(fragDirectionalLightsT[i], fragPositionT, normalT, fragViewDirectionT, albedo, metallic, roughness);
  		numLights++;
  	}
  }
  ...
  for (int i = 0; i < maxFragPointLights; i++) {
  	if (numLights < MAX_NUM_FRAG_LIGHTS) {
  		result += calcPointLight(fragPointLightsT[i], fragPositionT, normalT, fragViewDirectionT, albedo, metallic, roughness);
  		numLights++;
  	}
  }
  ...
  for (int i = 0; i < maxFragSpotLights; i++) {
  	if (numLights < MAX_NUM_FRAG_LIGHTS) {
  		result += calcSpotLight(fragSpotLightsT[i], fragPositionT, normalT, fragViewDirectionT, albedo, metallic, roughness);
  		numLights++;
  	}
  }
