...
vec3 calcLuminance(vec3 positionT, vec3 normalT, vec3 viewDirection, vec3 lightDirection, vec3 radiance, vec3 albedo, float metallic, float roughness) {
...
vec3 calcDirectionalLight(DirectionalLight directionalLight, vec3 positionT, vec3 normalT, vec3 viewDirection, vec3 albedo, float metallic, float roughness) {
	vec3 lightDirection = normalize(-directionalLight.direction);

	vec3 radiance = directionalLight.light.color;

	return calcLuminance(positionT, normalT, viewDirection, lightDirection, radiance, albedo, metallic, roughness);
}

vec3 calcPointLight(PointLight pointLight, vec3 positionT, vec3 normalT, vec3 viewDirection, vec3 albedo, float metallic, float roughness) {
...
vec3 calcSpotLight(SpotLight spotLight, vec3 positionT, vec3 normalT, vec3 viewDirection, vec3 albedo, float metallic, float roughness) {
	...
	float epsilon = abs(cos(spotLight.innerCutoffAngle) - cos(spotLight.outerCutoffAngle));

	if (epsilon == 0.0) {
		epsilon = 0.001;
	}

	float intensity = clamp((theta - cos(spotLight.outerCutoffAngle)) / epsilon, 0.0, 1.0);
...
void main()
{
  ...
  for (int i = 0; i < maxFragDirectionalLights; i++) {
    ...
    directionalLight.direction = normalize(tbn * vec3(view * vec4(directionalLight.direction, 0.0)));
  ...
  for (int i = 0; i < maxFragSpotLights; i++) {
    ...
    spotLight.pointLight.position = tbn * vec3(view * vec4(spotLight.pointLight.position, 1.0));
    spotLight.direction = normalize(tbn * vec3(view * vec4(spotLight.direction, 0.0)));
  ...
  // Compute vertex lighting in view space.
	...
	int maxVertDirectionalLights = min(numVertDirectionalLights, MAX_NUM_VERT_LIGHTS);
	for (int i = 0; i < maxVertDirectionalLights; i++) {
		if (numLights < MAX_NUM_VERT_LIGHTS) {
			result += calcDirectionalLight(vertDirectionalLights[i], vertPositionV, vertNormalV, viewDirection, albedo, metallic, roughness);
			numLights++;
		}
	}
	...
	for (int i = 0; i < maxVertPointLights; i++) {
		if (numLights < MAX_NUM_VERT_LIGHTS) {
			result += calcPointLight(vertPointLights[i], vertPositionV, vertNormalV, viewDirection, albedo, metallic, roughness);
			numLights++;
		}
	}
	...
	for (int i = 0; i < maxVertSpotLightsSpot; i++) {
		if (numLights < MAX_NUM_VERT_LIGHTS) {
			result += calcSpotLight(vertSpotLights[i], vertPositionV, vertNormalV, viewDirection, albedo, metallic, roughness);
			numLights++;
		}
	}
